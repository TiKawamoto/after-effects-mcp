...
- Panel waits for ownership: another panel may be active, or an earlier AE session may have exited without releasing ownership. On Windows the server checks at most every 30 seconds and releases the old owner only if all surviving AE processes started after its last heartbeat (or none remain). An old heartbeat alone never permits takeover. The current owner's last heartbeat is retained for this verification.
- To check abandoned ownership immediately, set `AE_MCP_BRIDGE_DIR` to the configured directory and run `npm run bridge-recover`. This works alongside the existing MCP client and running AE. Windows process-query failures, missing evidence and potentially surviving owners preserve ownership; use the closed-AE reset below if verification cannot succeed. Recovery preserves request/result/started journals and never replays edits. Restart an already-running MCP client after updating the code to activate automatic recovery.
